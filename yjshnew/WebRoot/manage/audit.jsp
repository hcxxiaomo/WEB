...
{	
	...
	switch(inttype)
	{
		case 1: sql =  "select * from yjsh_news  where type ='no' order by id desc ";
		item = "新闻动态";
		break;
		case 2: sql =  "select * from yjsh_notices  where type ='no' order by id desc ";
		item = "通知公告";
		break;
		case 3: sql =  "select * from yjsh_hhdjt  where type ='no' order by id desc ";
		item = "含弘大讲堂";
		break;	
		case 4: sql =  "select * from yjsh_hhxzjt  where type ='no' order by id desc ";
		item = "含弘学子讲堂";
		break;	
		case 5: sql =  "select * from yjsh_jobs  where type ='no' order by id desc ";
		item = "就业信息";
		break;	
		case 6: sql =  "select * from yjsh_practices  where type ='no' order by id desc ";
		item = "社会实践";
		break;						
	}
...
<script>
  ...
  <body style="background-color : #eef2fb">
		<div>
		<h4 style="color:red">待审核的<%=item %>后台管理</h4>
		<table style="font-size:14px;">
			...
			%>
			<tr>
				 <form method='post' action='manage/audit_modified.jsp' >
				...
				<input name="type" type="hidden"	value="<%=type%>" />
				 <input style="border-bottom:1px solid #005aa7; border-top:0px; border-left:0px; border-right:0px; background-color:transparent;" type="submit" name="submit" value="删除" />
				  ...
				  <input style="border-bottom:1px solid #005aa7; border-top:0px; border-left:0px; border-right:0px; background-color:transparent;" type="submit" name="submit" value="通过" />
				  </td>
				<%
					}
				%>
	...
	<div>
						<br/>	
								<div class="pagenumber">	
								  <a href=manage/audit.jsp?type=<%=inttype %>&page=1>首页</a>&nbsp; 
								   <a href=manage/audit.jsp?type=<%=inttype %>&page=<%=intPage-1 %>>上一页</a>&nbsp;
								   <%
								     for(int j=intPage-11;j<=intPage+12;j++){
								         ...
								         %>
								           <a href=manage/audit.jsp?type=<%=inttype %>&page=<%=j%>><%=j%></a>&nbsp;
								         <%
								         if(j>10)
								         break;
								     }
								   %>
								  <a href=manage/audit.jsp?type=<%=inttype %>&page=<%=intPage+1 %>>下一页</a>&nbsp;
								   <a href=manage/audit.jsp?type=<%=inttype %>&page=<%=intPageCount %>>尾页</a>&nbsp;	
					</div>
