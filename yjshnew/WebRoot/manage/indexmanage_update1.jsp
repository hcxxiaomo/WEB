...
<SCRIPT language=javascript>
	alert("不允许上传某些危险类型文件");
	window.location = 'indexmanage.jsp';
</script>
<%
}
		...
		//String file = su.getRequest().getParameter("file");
		String content = su.getRequest().getParameter("content1");
	 	...
	 	//file = new String(file.getBytes("utf-8"),"utf-8");
...
//   	     String content = request.getParameter("content1").toString();
	     ...
	     String sql_add = null;
	     if(file_size == 0 ){
	      sql_add = "insert into "+talbe+" (title,content,author,type,time) values('" +title+ "','" +content+ "','" +author+ "','yes','" +time+ "')";
	     }else{
	     sql_add = "insert into "+talbe+" (title,content,author,type,origanname,file,time) values('" +title+ "','" +content+ "','" +author+ "','yes','"+dbOriganName+"','" +dbsaveurl+ "','" +time+ "')";
		}
